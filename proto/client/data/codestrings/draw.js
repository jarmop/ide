...
static inline uint32_t blend_coverage_over_xrgb(uint32_t dst, uint8_t cov, uint8_t fr, uint8_t fg, uint8_t fb) {
    if (cov == 0) return dst;
 ...
 */
static void draw_glyph(struct bitmap *container, const FT_Bitmap *glyph, int dst_x, int dst_y, uint8_t fr, uint8_t fg, uint8_t fb) {
    /**
...
static void draw_text(struct bitmap *container, int baseline_x, int baseline_y, struct line *lines, int linecount, uint8_t fr, uint8_t fg, uint8_t fb) {
